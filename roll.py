...
from modules.utils import separator_line
...
def sum_rolls(rolls: int, faces: int, modifier: int):
...
def advantage(rolls: int, faces: int, modifier: int):
    ...
    expected_avg = (
        faces * rolls / (rolls + 1) + 0.5 + modifier
    )  # thank you, Matt Parker # m / (m + 1) * n + 0.5
...
def disadvantage(rolls: int, faces: int, modifier: int):
    ...
    expected_avg = (
        faces / (rolls + 1) + 0.5 + modifier
    )  # thank you, Matt Parker # 1 / (m + 1) * n + 0.5
...
def main(rolls: int, faces: int, op: Callable, modifier: int) -> None:
    ...
    print(intro_str)
    print(separator_line(len(intro_str), middle=["Good luck", "GL"]))

    raw_results, result, expected_avg = op(rolls, faces, modifier)
    print(
        "You rolled: " + ", ".join(map(str, raw_results))
    )  # all this just to get rid of the brackets. perhaps unwarranted.
    print(f"Result: {result}")
    print(
        f"With an expected value of {round(expected_avg, 2)}"
    )  # val:.2f will have trailing zeros. :.4g won't but converts to scientific notation if needed.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=20,
        help="number of faces/sides on the die (default: 20)",
    )
    parser.add_argument(
        "-m", "--modifier", type=int, default=0, help="modifier to add to the result"
    )
